...
import { ArrowRightLeft, Copy, Trash2, Type, AlertCircle, Check } from 'lucide-react';
...
export default function Base64Page() {
  ...
  // Auto-resize textarea logic could be added, but fixed height is fine for utilities.
  ...
  const handleEncode = () => {
    try {
      ...
      // UTF-8 Safe Encoding
      const encoded = btoa(String.fromCharCode(...new TextEncoder().encode(inputText)));
  ...
  const handleDecode = () => {
    try {
      ...
      // Robust Handling: Remove whitespace/newlines if requested
      if (ignoreWhitespace) {
        toDecode = toDecode.replace(/\s+/g, '');
      }

      // Check for valid base64 chars roughly before decoding to give better error?
      // Native atob will throw if invalid.
      ...
      setOutputText(decoded);
        ...
        console.error(err);
      setError('Failed to decode. Input might contain invalid characters or be corrupted.');
    }
  };

  // Trigger conversion when input or mode changes
  useEffect(() => {
  ...
  }, [inputText, mode, ignoreWhitespace]);
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-lg text-foreground/80">
          Encode and decode text safely. Handles Unicode and newlines robustly.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
            ...
            <textarea
                ...
                placeholder={mode === 'encode' ? "Type text to encode..." : "Paste Base64 string here..."}
            ...
            <div className="flex items-center justify-between p-4 bg-background rounded-xl border border-border">
                 ...
                 {mode === 'decode' && (
                     <label className="flex items-center gap-2 text-xs text-foreground/80 cursor-pointer select-none" title="Removes \n, \r, and spaces before decoding">
                         <input 
                            type="checkbox" 
